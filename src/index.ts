...
export class Client extends EventEmitter {
  ...
  constructor(clientOptions: ClientOptions) {
    ...
    ;(this.useTLS = clientOptions.useTLS || false), (this.rejectUnauthorized = clientOptions.rejectUnauthorized || true)
  ...
  private getHttpUrl(rootData: RootData) {
    ...
    if (!result) {
      throw new Error(`No signalk-http endpoint in rootData ${JSON.stringify(rootData)}`)
    }
    return result
  }
  ...
  private getWsUrl(rootData: RootData) {
    ...
    if (!result) {
      throw new Error(`No signalk-ws endpoint in rootData ${JSON.stringify(rootData)}`)
    }
    return result
  }
  ...
  getVesselsSelfPath(path: string, postPath = ''): Promise<any> {
    ...
    return this.rootData()
      .then((rootData) =>
        this.fetch(`${this.getHttpUrl(rootData)}vessels/self/${path.replace(/\./g, '/')}${postPath}`, { headers }),
      )
      .then((r: Response) => {
        if (r.status !== 200) {
    ...
    return this.rootData().then(
      (rootData) => {
        ...
        if (isNode && this.useTLS && this.rejectUnauthorized === false) {
          ...
          if (this.cookieJar) {
            debug('cookies')
            this.cookieJar.getCookiesSync(wsUrl).forEach((cookie) => debug('cookie:', cookie))
          }
  ...
  disconnect(): Promise<void> {
    ...
    if (!this.ws) {
      return Promise.reject('Can not disconnect because websocket not connected')
    }
    return new Promise((resolve, reject) => {
